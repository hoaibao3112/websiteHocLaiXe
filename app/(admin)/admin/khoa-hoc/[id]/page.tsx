...
export default function EditCoursePage({ params }: PageProps) {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        if (error || !courseItem) {
          alert("Không tìm thấy thông tin khóa học cần chỉnh sửa.");
        ...
        reset({
          ...
          class_code: courseItem.class_code,
          description: courseItem.description || "",
          original_price: courseItem.original_price,
          sale_price: courseItem.sale_price,
          image_url: courseItem.image_url,
          badge: courseItem.badge || "",
          is_active: courseItem.is_active,
          display_order: courseItem.display_order,
          features: courseItem.features && courseItem.features.length > 0
            ? courseItem.features
            : [""],
        });
      } catch (err) {
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      ...
      alert("Đã xảy ra lỗi khi cập nhật khóa học. Vui lòng thử lại.");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  if (fetching) {
    return (
      <div className="flex flex-col items-center justify-center py-32 gap-3">
        ...
        <p className="text-sm text-neutral-500 font-medium">Đang tải thông tin khóa học...</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8 max-w-5xl">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="font-display text-3xl font-bold text-neutral-900">Chỉnh sửa khóa học</h1>
          <p className="text-neutral-500 mt-1">Cập nhật thông tin, học phí và các quyền lợi của khóa học.</p>
        </div>
      </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white p-6 rounded-2xl border border-neutral-100 shadow-sm space-y-5">
            ...
            <div className="border-t border-neutral-100 pt-5 flex gap-3">
              <Button type="submit" className="flex-1 inline-flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white" disabled={submitting}>
                {submitting ? (
